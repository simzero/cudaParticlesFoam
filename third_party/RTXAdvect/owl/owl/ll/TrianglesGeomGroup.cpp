...
namespace owl {
  namespace ll {
      ...
      void dbgPrintBVHMem(size_t numItems,
    ...
    void TrianglesGeomGroup::buildAccel(Context *context) 
    {
      ...
      dbgPrintBVHMem(/*numItems*/
          ...
          /*finalBVHSize*/
          compactedSizeBuffer.sizeInBytes
      );
      size_t peak_mem = std::max(tempBuffer.sizeInBytes + outputBuffer.sizeInBytes, outputBuffer.sizeInBytes + compactedSizeBuffer.sizeInBytes);
      std::cout << OWL_TERMINAL_YELLOW
          << "@owl: peak mem= "
          << prettyNumber(peak_mem)
          << OWL_TERMINAL_DEFAULT
          << std::endl;
      // ==================================================================
      // aaaaaand .... clean up
